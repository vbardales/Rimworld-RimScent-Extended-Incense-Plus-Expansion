...
remaining:
  ...
  - defect: public silent status lacks the required " (unofficial)" title suffix and disclaimer.
  - defect: About.xml lacks the required final GitHub source-code link.
  - defect: CHANGELOG.md and test scenarios/results are absent.
...
required `(unofficial)` suffix and opening disclaimer are absent from About.xml and README. The
English About description also lacks the required final
`[url=URL_DU_DEPOT]Source code on GitHub[/url]` link.
...
`TEST_SCENARIOS.md`, `Tests/`, or `CHANGELOG.md` is present, so the later preTest/done gates are
not met.
